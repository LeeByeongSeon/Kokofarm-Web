...
<script language="javascript">
	...
	function get_detail_info(index, rowData, $detail){		// + 펼칠 때 이벤트
		...
		$.ajax({url:'0109_action.php',data:data_arr,cache:false,type:'post',dataType:'json',
			success: function(data) {
					...
					popup_confirm("재산출 요청 " + prop, data.fdName + " 농장의 재산출 요청을 " + prop + "하시겠습니까? ", 
						function(confirm){
							if(confirm){
								...
								$.ajax({url:'0109_action.php',data:confirm_data,cache:false,type:'post',dataType:'json',
									success: function(data) {
										switch(data.result){
											case "ok":
												search_action("search");
